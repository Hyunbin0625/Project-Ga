...
public class PlatformSpawner : MonoBehaviour
{
    ...
    public GameObject bossStagePrefab;  // 생성할 보스 맵의 원본 프리팹
    public int maxPlatforms = 15;       // 최대 플랫폼 개수
    ...
    private int currentMaxPlatforms = 3; // 최대 플랫폼 개수
    ...
    void SpawnPlatform()
    {
        ...
        if (platforms.Count < currentMaxPlatforms)
        {
            ++platformCount;    // 플랫폼 카운트 증가
    ...
    void SpawnBossStage()
    {
        ...
        // 위치 설정
        Vector3 bossStagePos = lastOrigin.transform.position;
        bossStagePos += new Vector3(bossStagePrefab.transform.lossyScale.x / 2 + 7.5f, -4, 0);

        // 플랫폼 생성
        GameObject bossStage = Instantiate(bossStagePrefab, bossStagePos, Quaternion.identity);

        // 충돌 감지용 Zone에 PlatformTrigger의 spawner 변수 설정
        B_PlatformTrigger platformTrigger = bossStage.GetComponentInChildren<B_PlatformTrigger>();
        if (platformTrigger != null)    // 플랫폼 트리거 재대로 생성되었다면
        {
            platformTrigger.spawner = this; // 자식, 플랫폼 트리거의 spawner에 자신(PlatformSpawner) 저장
            platformTrigger.SetInitialPlayerRotation(lastPlatform.GetComponentInChildren<PlatformTrigger>().GetPlatformRotation());
    ...
    public void TrySpawnPlatform()
    {
        ...
        if (platformCount < maxPlatforms)
        {
            ...
            if (platforms.Count >= currentMaxPlatforms)
            {
                ...
                platforms.RemoveAt(0);  // 뒤에 값을 앞으로 땡겨, 삭제된 0번 인덱스 공간을 채우는 함수
            }

            SpawnPlatform();    // 플랫폼 생성
        }
        else if (platformCount == maxPlatforms)
        {
            SpawnBossStage();   // 보스 스테이지 생성
        }
    }
    ...
    public void RemoveAllPlatforms()
    {
        ...
        {
            // 플랫폼에 생성된 몬스터를 삭제
            monsterSpawner.RemoveMonstersForPlatform(platform);
